...
- **Workflow Issue 2 — Pipeline cancellation flow**: Fully resolved. Decisions:
  - Trigger: `pipeline_cancel_requested` on `system:pipelines` (TUI → API → Redis → orchestrator).
  - Cooperative cancellation (P9 — Stability Over Wasted Work): orchestrator
    stops dispatching, running agents finish their current atomic operation and
    exit naturally. No SIGTERM, no timeout, no forced kill. Agents don't know
    they've been cancelled.
  - New transitional state: `CANCELLING` (waiting for containers to exit) →
    `CANCELLED` (all exited, finalized).
  - PRs left open after cancellation for human review. Separate "Clean up"
    action via TUI closes PRs and deletes branches (spawns Richelieu).
  - Resumability out of scope. `CANCELLED` is terminal. Noted in
    `docs/future-work.md`.
  - Consistency fixes in spec 13: `FAILED` → `PARTIALLY_FAILED` (P8),
    `CONTROLLER_` → `ORCHESTRATOR_` env prefix.
  - New principle: P9 (Stability Over Wasted Work) in knowledge base.
  - Created `docs/future-work.md` for deferred enhancement ideas.
  - Updated specs: 01, 02, 13, 14, 15.
- **Workflow Issue 3 — NEEDS_HUMAN notification mechanism**: Fully resolved. Decisions:
  - Configurable webhook notifications for out-of-band alerting (Slack, email,
    PagerDuty). Deployment-level config via `.env` + runtime update via API.
    Fire-and-forget POST with JSON payload. Grafana alerting documented as
    ops-native alternative.
  - TUI Dashboard as default screen (replaces Pipeline Overview). Three panels:
    Attention Queue (top), Active Pipelines (middle), System Health (bottom).
  - Attention Queue is a persistent, inspectable panel — not a dismissable banner.
    Severity levels (HIGH/MED/LOW). Auto-resolves on state change. Dedicated
    drill-down screen with in-place actions per item type.
  - `system:escalations` Redis Stream as the single event bus for both SSE
    (TUI) and webhooks (external).
  - Attention item types: needs_human, review_flagged, consensus_failed,
    pipeline_partial_fail, budget_warning, budget_critical.
  - API endpoints: `GET /attention`, `DELETE /attention/:id`,
...
**Issue 10 — Confidence score semantics.**
Renamed from "human review score" to "confidence score" (0.0–1.0). Higher = more
confident = auto-approve. Score < threshold → human review required. Threshold
default: 0.7. Still needs deeper investigation: score component weights, how
novelty/complexity/risk combine into composite confidence, learning dynamics.
